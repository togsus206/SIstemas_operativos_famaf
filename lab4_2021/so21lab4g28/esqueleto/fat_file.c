...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    ...
    //Agregamos este condicional para que otros sistemas de archivos ignoren al logfile
...
void fat_file_remove(fat_file file, fat_file parent) {
    u32 current_cluster = 0, next_cluster = 0, final_cluster_to_rm = 0;

    //asignamos el primer cluster del archivo
    ...
    fat_table_set_next_cluster(file->table, current_cluster,
                               FAT_CLUSTER_END_OF_CHAIN);
    final_cluster_to_rm = current_cluster;
    ...
    fat_table_set_next_cluster(file->table, final_cluster_to_rm,
                               FAT_CLUSTER_FREE);

    for (int i = 0; i< 8;i++){
        file->dentry->base_name[i] = FAT_FILENAME_DELETED_CHAR;
    }
    write_dir_entry(parent, file->dentry, file->pos_in_parent);
}
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    u32 cluster = 0;
    u32 next_cluster =0; //vemos cual sera su proximo cluster
    ...
    while (bytes_remaining > 0 && !fat_table_is_EOC(file->table, cluster)) {
        ...
        DEBUG("Estoy en mi cluster: %u", file->file.num_clusters);
        if (bytes_remaining > 0) {
            //Implementacion para seguir asignando clusters                                     // mientras haya bytes a esscribir
            if(fat_table_is_EOC(file->table,fat_table_get_next_cluster(file->table,cluster))){  // Si no es el final de la fat table
            next_cluster = fat_table_get_next_free_cluster(file->table);                        // Buscamos el primer cluster libre en la tabla
            ...
            DEBUG("Mi cluster final ahora es: %u", file->file.num_clusters);
            }
            //Fin
        }
    }
